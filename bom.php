            ...
            <!--START OF FIRST TABLE-->    
                  <table id="example2" class="table table-bordered table-striped">
                    ...
                    <tr>
                        ...
                        <th style='width:12%'>Marked-up Price</th>
                        <th style='width:12%'>Selling Price (w/ VAT)</th>
                        <th style='width:12%'>Total Price VAT-Ex</th>
                        <th style='width:5%'></th>
                        </center>
                      </tr>
                    </thead>
                    ...
                    <?php
                        if($bom->get_foreign($_GET['bom']) > 0){
                            $bom->show_foreign($_GET['bom']);   
                        }
                        else{
                    ?>
                      ...
                      <tr id='tr1'>
                        ...
                        <td>
                          <input type="text" id='mu_price1' name='mu_price[]' style="text-align:right; background-color:transparent; border:transparent; width:100%" required class="form-control">
                        </td>
                        <td>
                          <input type="text" id='unitprice1' name='price[]' style="text-align:right; background-color:transparent; border:transparent; width:100%" required class="form-control">
                        </td>
                        <td>
                    ...
                    <tfoot>
                      <tr>
                        <th colspan='6' id='vat_desc' style='text-align: right'>Total (VAT Exclusive)</th>
                  ...
                  <div style="clear:both; height:15px;"></div>
                                    ...
                                    <button type="submit" onclick='return check()' name="foreign" id="submit_new1" class="btn btn-primary" style="float:right; margin-top:2px; margin-right:10px"><i class='fa fa-save'></i> &nbsp;Submit</button>
  ...
  for (i = 1; i <= length; i++){
      add_function(i);
                    ...
                    <?php
                        if($bom->get_foreign($_GET['bom']) > 0){
                    ?>
                        var last_id = '<?php echo $bom->get_foreign($_GET['bom'])+1; ?>';
                    <?php    
                        }
                        else{
                    ?>
...
function add_function(id){
  $('#quan'+id).keydown(function (e) {
    ...
    $('#mu_price'+id).keydown(function (e) {
        // Allow: backspace, delete, tab, escape, enter and .
        if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
             // Allow: Ctrl+A, Command+A
            (e.keyCode === 65 && (e.ctrlKey === true || e.metaKey === true)) ||
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                if(data != '###'){
                    ...
                    $("#unitprice"+id).attr('readonly', true);
            }
            }
        });
    });
...
function multiply(id){
  ...
  var price_ = ($("#mu_price"+id).val()).replace(/\,/g,'');
...
<?php
if (isset($_GET['success_foreign'])) {
  ?>
    swal("Success", "Successfully recorded foreign components.", "success");
    history.pushState(null, null, 'foreign?bom=<?php echo $_GET['bom']; ?>');
<?php
}
?>
</script>
