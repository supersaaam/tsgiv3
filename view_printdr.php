...
if($som->check_dr($so)){ //may dr na
     //redirect to printing of DR
     echo "
     <script>
     window.location.href='print_dr.php?so=$so';
     </script>
     ";
}
else{ //dr is null
...
@keyframes spin {
  ...
  100% { transform: rotate(360deg); }
                 ...
                 <div class="form-group">
                    ...
                    <span style='float:right;' id='dr'></span>
                    </div>
                    
                    <input type='text' maxlength='100' 
                    onkeypress="return isNumberKey(event)" 
                    onkeyup="loading('dr')" id='dr_input' value='' required name="dr" class="form-control" placeholder='Type input here...'>
                  </div>           
                  ...
                  <button type="submit" name='save_dr'
	    ...
	    if (str.length == 0) { 
	        ...
	        xmlhttp.onreadystatechange = function() {
	            if (this.readyState == 4 && this.status == 200) {
					...
					if(this.responseText == " &nbsp; DR Number already exists! "){
						$("#dr").attr("style", "color: red; font-weight: bold; float:right;");
						$("#submit").attr("disabled", true);
					}
					else{
						$("#dr").attr("style", "color: green; font-weight: bold; float:right;");
						$("#submit").attr("disabled", false);
					}
	            }
	        };
	        xmlhttp.open("GET", "models/validate_ajax.php?q_dr=" + str, true);
	        xmlhttp.send();
	    }
	}
