...
if(isset($_POST['save_only']) || isset($_POST['save_print'])){
    ...
    $ins = $con->prepare('UPDATE tbl_sales_order SET DebitStatus=?, DebitAmount=DebitAmount+?, Balance=Balance+? WHERE SONumber=?');
    $s = 'YES';
    ...
    $con = new mysqli($server, $user, $pw, $db);
    $ins_c = $con->prepare('INSERT INTO `tbl_debit`(`MemoNumber`, `SONumber`, `DebitDate`, `Total`) VALUES (?, ?,?,?)');
    ...
    foreach($desc as $k=> $d){
        if($d != null){
            ...
            $con = new mysqli($server, $user, $pw, $db);
            $ins_ap = $con->prepare('INSERT INTO `tbl_debit_bd`(`MemoNumber`, `Quantity`, `Description`, `UnitPrice`, `Amount`) VALUES (?, ?, ?, ?, ?)');
...
if(isset($_POST['save_print'])){
    //for printing
    header('location: ../print_dmemo.php?mn='.$memonumber);
}
elseif(isset($_POST['save_only'])){
    header('location: ../sidr_records?success_debit');
}
?>