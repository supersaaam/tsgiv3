...
if($_FILES['docs']['error'] !== UPLOAD_ERR_OK){
 ...
 for($i=0;$i<$countfiles;$i++){
    if($_FILES["docs"]["type"][$i] == "image/gif"
	|| $_FILES["docs"]["type"][$i] == "image/png"
	|| $_FILES["docs"]["type"][$i] == "image/jpeg"
	|| $_FILES["docs"]["type"][$i] == "image/jpg"
	|| $_FILES["docs"]["type"][$i] == "image/pjpeg"){

        $inv = $_POST['inv'];
        $uploaddir = '../images/attachments/'.$inv;
...
echo $_FILES['docs']['tmp_name'][$i];
   ...
   //insert to database
   $stmt = $con->prepare('INSERT INTO `tbl_imp_docs`(`ProformaInvNo`, `DocumentPath`, `FileTitle`) VALUES (?, ?, ?)');
   $title = basename($_FILES['docs']['name'][$i]);
   $stmt->bind_param('sss', $inv, $uploadfile, $title);
   $stmt->execute();
   
    }
 }      
        echo"
        <script>window.location.href='../attachment?inv=$inv&uploaded';</script>
        ";
}
else{
    //redirect to page -> not uploaded
    echo"
        <script>window.location.href='../attachment?inv=$inv&not_uploaded';</script>
        ";
}
}
elseif(isset($_GET['delete'])){
    ...
    $stmt = $con->prepare('DELETE FROM `tbl_imp_docs` WHERE `ImpDocs_ID`=?');
    $stmt->bind_param('i', $id);
    if($stmt->execute()){
        echo"
        <script>window.location.href='../attachment?inv=$inv&deleted';</script>
        ";
    }
}
         ...
         if($stmt->num_rows > 0){
            while($stmt->fetch()){
                ...
                <tr>
                    ...
                    <td><a href='models/attachment_model.php?delete&id=$id&inv=$invnum'><button class='btn btn-danger btn-sm'><i class='fa fa-trash'></i>&nbsp;&nbsp;Delete File</button></a></td>
                </tr>
                ";    
            }
         }
