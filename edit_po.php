...
<head>
  ...
  <title>Transcendo | PO Module</title>
      ...
      <div class="box-body">
              ...
              <form role="form" action="models/po_aed.php" method="post" id="">
                    ...
                    <div class="form-group" style='float:left; width:48%; margin-right:2%'>
                      ...
                      <input required name="po_num" value='<?php echo $po_n; ?>' class="form-control" readonly>
                    </div>
                   ...
                   <div class="form-group">
                    ...
                    <div class="form-group">
                     ...
                     <input list="cperson_list" value='<?php echo $po->cperson; ?>' name="cperson" id='cperson' class="form-control" placeholder="Please input contact person's name ...">
    		        ...
    		        <div class="form-group" style='width: 73%; margin-right: 2%; float:left'>
                      ...
                      <input list="terms_list" required name="terms"  value='<?php echo $po->terms; ?>' class="form-control" placeholder="Please input terms ...">
                    </div>
                    ...
                    <div class="form-group" style='width: 23%; margin-left: 2%; float: left'>
                      ...
                      <select name='currency' class='form-control' required>
                          <?php
                          function selected($currency, $po_currency){
                          ...
                          function mode_selected($shipment, $po_shipment){
                              if($shipment == $po_shipment){
                                  return ' selected ';
                              }
                              else{
                                  return '';
                          ...
                          function type_selected($type, $po_type){
                              if($type == $po_type){
                                  return ' checked ';
                              }
                              else{
                          ...
                          ?>
                          <option value='USD' <?php echo selected('USD', $po->c); ?>>USD</option>
                          <option value='EURO' <?php echo selected('EURO', $po->c); ?>>EURO</option>
                          <option value='PHP' <?php echo selected('PHP', $po->c); ?>>PHP</option>
                          <option value='AUD' <?php echo selected('AUD', $po->c); ?>>AUD</option>
                      </select>
                    </div>
                    
                    <div class="form-group" style='width: 73%; margin-right: 2%; float:left'>
                      <label>Incoterms</label>
                      <input type='text' required name="incoterms"  value='<?php  echo $po->incoterms; ?>' class="form-control" placeholder="Please input terms ...">
                    </div>
                    
                    <div class="form-group" style='width: 23%; margin-left: 2%; float: left'>
                      <label>Shipment</label>
                      <select name='shipment' class='form-control' required>
                          <option value='Air' <?php echo selected('Air', $po->shipment); ?>>Air</option>
                          <option value='Sea' <?php echo selected('Sea', $po->shipment); ?>>Sea</option>
                          <option value='To be determined' <?php echo selected('To be determined', $po->shipment); ?>>To be determined</option>
                      </select>
                    </div>
                    ...
                    <div class="form-group" style='width: 48%; margin-right: 2%; float:left'>
                      <label>PO Type:</label>
                      <br>
                      <input type='radio' <?php echo type_selected('Local', $po->type); ?> class='type' name='type' value='Local'>&nbsp; Local
                      <input type='radio' <?php echo type_selected('Foreign', $po->type); ?> class='type' style='margin-left: 20px;' name='type' value='Foreign'>&nbsp; Foreign
                      
                      <br><br>
                      <div id='type'>
                      <label>Remaining AR</label>
                      <input type='number' step='any' max='' min='0' name="remaining_ar"  value='<?php echo $po->ar; ?>' class="form-control" placeholder="Please input amount ...">
                      </div>
                    ...
                    <div class="form-group" style='width: 48%; margin-left: 2%; float:left'>
                      <label>Note</label>
                      <textarea maxlength='500' name="note" class="form-control" style='width:100%; resize: none;' rows='5'><?php echo $po->note; ?></textarea>
                    </div>
                  ...
                  <table id="example2" class="table table-bordered table-striped">
                    ...
                    <tbody id="trans_table">
                      <?php
                        $po->show_bd_po_edit($po_n);
                      ?>
                    </tbody>
                    <tfoot>
                      <tr>
                        <th colspan='6' style='text-align: right'>Total Amount</th>
                        <td><input type="text" name="total_price" id='total_price' style="text-align:left; background-color:transparent; border:transparent" class="form-control" readonly value='<?php echo $po->total_amount; ?>'></td>
                      </tr>
                    </tfoot>
                  </table>
                  
                    <div style="clear:both; height:15px;"></div>
                  <button type="submit" name="edit_submit" id="submit_new1" class="btn btn-primary" style="float:right; margin-top:2px; margin-right:10px"><i class='fa fa-save'></i> &nbsp;Submit</button>
                </form>
...
$('input[type=radio][name=type]').change(function() {
    if (this.value == 'Foreign') {
        $("#type").html("<label>Remaining AR</label><input type='number' step='any' max='' min='0' name='remaining_ar'  value='<?php echo $po->ar; ?>' class='form-control' placeholder='Please input amount ...'>");
    }
    else if (this.value == 'Local') {
        $("#type").html("<label>Delivery Address</label><input type='text' name='del_address'  value='<?php echo $po->del_add; ?>' class='form-control' placeholder='Please input delivery address ...' required>");
    }
});
